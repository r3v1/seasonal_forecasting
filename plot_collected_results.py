...
def main(y_var, area, exp, src, in_dir, out_dir):
    ...
    skill_scr_files = out_dir.glob('skill_scores_' + data['basename'][:-6] + '*csv')
    ...
    for i, fle in enumerate(skill_scr_files):
        ...
        p = list(cols.keys())
        for ptn in p:
            result = re.search(ptn, fle.as_posix())
            if result:
                y_area = ptn
    ...
    files = out_dir.glob(data['basename'][:-6] + '*pkl')
    for fle in files:
    ...
    for i, var in enumerate(data['X_vars']):
        ...
        for j, cp in enumerate(np.arange(ncomps)):
            ...
            lsmask = fcts.read_and_select(in_dir / 'lsmask_era20c_monthly_1900-2010.nc', 'lsm', -99)
            if (nme == 'SST'):
