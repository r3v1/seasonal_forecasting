...
def main(y_var, area, exp, src, in_dir: Path, out_dir: Path):
    ...
    for s, ssn in enumerate(data['seasons']):
        ...
        to_plot = fcts.count_prdctr_freqs(results[ssn_idx]['Optimal predictors'], 'Predictor')
        g = sns.barplot(x='Predictor', y='Frequency (%)', data=to_plot[:15], ax=axes[0, s], palette='Reds_r')
        g.set_xticklabels(g.get_xticklabels(), rotation=90)
        g.set_title(ssn)

        to_plot = fcts.count_prdctr_freqs(results[ssn_idx]['N of optimal predictors'], 'N of predictors').sort_index()
    ...
    fig1.savefig(out_dir / ('fig_prd_counts_' + data['basename'] + '.pdf'), bbox_inches='tight')
    # plt.show()

    # Calculate and save skill scores, and plot time series graphs
    ...
    for s, ssn in enumerate(data['seasons']):
        ssn_idx = results['Season'] == ssn
        all_idx = fcts.bool_index_to_int_index(
            np.isin(data['Y']['time.season'], ssn) & np.isin(data['Y']['time.year'], data['all_yrs']))
        trn_idx = fcts.bool_index_to_int_index(
            np.isin(data['Y']['time.season'], ssn) & np.isin(data['Y']['time.year'], data['trn_yrs']))
        tst_idx = fcts.bool_index_to_int_index(
            np.isin(data['Y']['time.season'], ssn) & np.isin(data['Y']['time.year'], data['tst_yrs']))
        ...
        for m, model in results[ssn_idx][:n_models].iterrows():
            ...
            if (data['experiment'] == 'SKIPQMAP'):
                ...
                forecast[all_idx, i] = fcts.q_mapping(observations[trn_idx], forecast[trn_idx, i], forecast[all_idx, i],
                                                      100)
        ...
        persistence[all_idx] = fcts.q_mapping(observations[trn_idx], observations[trn_idx - 1],
                                              observations[all_idx - 1], 100)
        ...
        axes1[s].plot(dtaxis, persistence[all_idx], c='k', linestyle='--', linewidth=0.8,
                      label='Persistence, ACC=' + str(round(corp[1], 2)))
        ...
        axes1[s].set_xlim([pd.Timestamp('1940-10-31 00:00:00'), pd.Timestamp('2012-10-31 00:00:00')])
        # axes1[s].set_ylim([-3, +3])

    plt.tight_layout()
    fig1.savefig(out_dir / ('fig_probab_nmodels' + str(ens_size) + '_' + data['basename'] + '.png'), dpi=120)
    fig1.savefig(out_dir / ('fig_probab_nmodels' + str(ens_size) + '_' + data['basename'] + '.pdf'))
    # plt.show()
    ...
    for s, ssn in enumerate(data['seasons']):
        ...
        if (ssn == 'JJA'):
            axes1[0].axvline(x=data['Y']['time'][trn_idx][-1].values, c='k', linewidth=2.0)
    ...
    axes1[0].set_ylabel('ACC')
    axes1[1].set_ylabel('ACC')
    axes1[0].set_xlabel('Year')
    axes1[1].set_xlabel('Ensemble size')
    axes1[1].set_ylabel('ACC')
    plt.tight_layout()
    plt.legend(loc='lower right')
    fig1.savefig(out_dir / ('fig_movingACC_nmodels' + str(ens_size) + '_' + data['basename'] + '.png'), dpi=120)
    fig1.savefig(out_dir / ('fig_movingACC_nmodels' + str(ens_size) + '_' + data['basename'] + '.pdf'))
    # plt.show()
